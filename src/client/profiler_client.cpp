...
void ProfilerClient::set_record_job_events() {
    c2s_channel_->acquire_writer_lock();

    c2s_channel_->write(MsgType::PROFILER_CMD);
    c2s_channel_->write(ProfilerMsgType::SET_RECORD_JOB_EVENTS);

    c2s_channel_->release_writer_lock();
}

void ProfilerClient::unset_record_job_events() {
    c2s_channel_->acquire_writer_lock();

    c2s_channel_->write(MsgType::PROFILER_CMD);
    c2s_channel_->write(ProfilerMsgType::UNSET_RECORD_JOB_EVENTS);

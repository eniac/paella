...
SchedulerFull2::SchedulerFull2(float unfairness_threshold, float eta) :
        server_(nullptr),
    ...
    SPDLOG_INFO("Setting up LLIS scheduler (full2)...");
...
void SchedulerFull2::try_handle_block_start_finish() {
...
void SchedulerFull2::handle_block_start_finish() {
...
void SchedulerFull2::handle_block_start(const job::InstrumentInfo& info) {
...
void SchedulerFull2::handle_block_finish(const job::InstrumentInfo& info) {
    job::Job* job = job_id_to_job_map_[info.job_id].get();
...
void SchedulerFull2::handle_mem_finish() {
    job::Job* job;
...
void SchedulerFull2::handle_new_job(std::unique_ptr<job::Job> job_) {
...
void SchedulerFull2::schedule_job() {
    schedule_mem_job();
    schedule_comp_job();
}

void SchedulerFull2::schedule_comp_job() {
...
void SchedulerFull2::schedule_mem_job() {
...
void SchedulerFull2::update_deficit_counters(job::Job* job_scheduled) {
...
double SchedulerFull2::calculate_priority(job::Job* job) const {
    return calculate_packing(job) - eta_ * server_->get_job_remaining_rl(job, job->get_cur_stage() + 1);
}

double SchedulerFull2::calculate_packing(job::Job* job) const {

...
for seed in {1,}; do
    for i in {3000,6000,8000,10000,12000,14000,16000,18000,20000,22000}; do
        ...
        ../../tests/client/test_client_concurrent_run_latencies_fixed_num_multi \
            ...
            --iat_g \
            --ln_sigma_n \
            "${abs_path}/release/jobs/tvm_mobilenet/libjob_tvm_mobilenet.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_densenet121/libjob_tvm_densenet121.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_googlenet/libjob_tvm_googlenet.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_inception_v3/libjob_tvm_inception_v3.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_resnet18/libjob_tvm_resnet18.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_resnet34/libjob_tvm_resnet34.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_resnet50/libjob_tvm_resnet50.so" 0.125 15 \
            "${abs_path}/release/jobs/tvm_squeezenet1_1/libjob_tvm_squeezenet1_1.so" 0.125 15
        wait
    done
done
